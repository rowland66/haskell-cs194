...
letter :: State String Char
letter = State $ parseLetter
    where parseLetter [] = Nothing
          parseLetter s@(c:cs)
              | isLetter c = Just (c, cs)
              | otherwise = Nothing
...
parseExp :: (Integer -> a) -> (String -> a) -> (a -> a -> a) -> (a -> a -> a) -> String -> Maybe a
parseExp con var add mul = (convert <$>) . evalParser (parseExpr <* eof)
